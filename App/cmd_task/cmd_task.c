...
void MotorCMDTask(void)
{
    ...
    if (++loop_count >= 2) {
        loop_count      = 0;
        cmd_tx.position = motor_data.components.encoder->angle;
        cmd_tx.velocity = motor_data.components.encoder->vel_estimate_ * motor_data.components.encoder->dir * 16384 / MOTOR_VEL_LIMIT;
